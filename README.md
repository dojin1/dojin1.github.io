#### 작업예정.
- 1달간은(프론트엔드) 주로 UI 계속진행(VS code개발환경) 하면서,
...
#### 20210524(월) 작업내역.
- 폰트어썸(아이콘웹폰트)장점:아이콘을 확대해도 깨지지않음(벡터방식의 아이콘)
- 포토샵 디자인으로 아이콘을 만들면 확대시 깨짐.(스칼라방식의 아이콘 벡터와반대.)
- 부트스트랩 레이아웃 : 화면을 12개의 컬럼으로 분리해서 크기를 반응형으로 만듭니다.
- row(가로줄), cols(세로칸), col-md-12(화면가로 크기를 12로지정 = 100%)
